<template>
    <a-drawer v-model:visible="visible" title="请输入内容" width="720" :closable="false" placement="left"
        :header-style="{ height: '40px' }" :footer-style="{ textAlign: 'right', padding: '8px' }"
        :body-style="{ padding: '5px 10px', marginBottom: '49px'}">
        <div class="main_area">
            <!-- 表单区域 -->
            <a-form ref="formRef" :model="form" :rules="rules" layout="vertical" autocomplete="off" @finish="onFinish"
                @finishFailed="onFinishFailed">
                <a-row :gutter="16">
                    <!-- 日期选择 -->
                    <a-col :span="12">
                        <a-form-item has-feedback label="日期:" name="date">
                            <a-date-picker :locale="locale" v-model:value="form.date" style="width: 100%"
                                placeholder="选择日期" value-format="YYYY-MM-DD" :get-popup-container="
                    ...
                    <a-col :span="12">
                        <a-form-item has-feedback label="体重(kg):" name="weight">
                            <a-input v-model:value.number.trim="form.weight" placeholder="输入体重" />
                        </a-form-item>
                    </a-col>
                </a-row>
                <a-row :gutter="16">
                    <!-- 训练类型 -->
                    <a-col :span="12">
                        <a-form-item has-feedback label="训练类型:" name="training">
                            <a-select v-model:value="form.training" mode="multiple" placeholder="选择训练类型"
                                :options="trainingType" @change="handleChange"></a-select>
                        </a-form-item>
                    </a-col>
                    <!-- 运动时长 -->
                    <a-col :span="12">
